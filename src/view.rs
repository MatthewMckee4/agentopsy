...
pub fn render(dashboard: &Dashboard) -> Result<String, fmt::Error> {
    ...
    let split_tool = saturating_sum(
        dashboard
            .sessions
            .iter()
            .flat_map(|session| session.turns.iter())
            .filter_map(|turn| turn.model_duration_ms.map(|_| turn.tool_duration_ms)),
    );
    let open_sessions = dashboard
        ...
        .count();
    let unestimated_turns = dashboard
        .sessions
        .iter()
        .map(|session| session.diagnostics.unestimated_turns)
        .fold(0, usize::saturating_add);
    let estimable_turns = dashboard
        .sessions
        .iter()
        .flat_map(|session| session.turns.iter())
        .filter(|turn| turn.model_duration_ms.is_some())
        .count();
    let (model_value, model_note) = model_metric(total_model, estimable_turns, unestimated_turns);
...
<main id="main">
<section class="page-heading"><div><p class="eyebrow">Overview</p><h1>Session activity</h1><p>Active time is recorded separately from wall containment. Tool time comes from matched timestamps; model time is estimated only when those receipts agree.</p></div></section>
...
<div class="metric model"><span>Model time</span><strong>{}</strong><small>{}</small></div>
...
<div class="metric"><span>Sessions</span><strong>{}</strong><small>{} open turns · {} anomalies</small></div>
    ...
    render_split_bar(&mut html, total_model, split_tool, unestimated_turns)?;
...
fn render_session_metrics(html: &mut String, session: &Session) -> fmt::Result {
    let estimable_turns = session
        .turns
        .iter()
        .filter(|turn| turn.model_duration_ms.is_some())
        .count();
    let (model_value, model_note) = model_metric(
        session.model_duration_ms,
        estimable_turns,
        session.diagnostics.unestimated_turns,
    );
    write!(
        html,
        r#"<div class="session-metrics"><div><span>Model</span><strong>{}</strong><small>{}</small></div><div><span>Tools</span><strong>{}</strong><small>timestamp-derived</small></div><div><span>Wall span</span><strong>{}</strong><small>includes gaps</small></div><div><span>Last activity</span><strong class="timestamp">{}</strong><small>UTC</small></div></div>"#,
        model_value,
        escape(&model_note),
...
fn model_metric(
    duration_ms: u64,
    estimable_turns: usize,
    unestimated_turns: usize,
) -> (String, String) {
    if unestimated_turns == 0 {
        return (duration(duration_ms), "estimated".to_owned());
    }
    if estimable_turns == 0 {
        let unit = if unestimated_turns == 1 {
            "turn"
        } else {
            "turns"
        };
        return (
            "—".to_owned(),
            format!("unavailable · {unestimated_turns} {unit}"),
        );
    }
    (
        duration(duration_ms),
        format!("partial · {unestimated_turns} unavailable"),
    )
}
...
fn render_timeline(html: &mut String, session: &Session) -> fmt::Result {
    html.push_str(
        r#"<section class="timeline-block"><div class="subhead"><div><h3>Turn containment timeline</h3><p>Neutral tracks show wall bounds. Labels show recorded active time; tool overlays follow matched timestamps.</p></div><div class="legend"><span>Neutral · wall span</span><span><i class="dot tool-dot"></i>Tool timestamp-derived</span></div></div><div class="turns">"#,
    );
...
fn render_turn(html: &mut String, turn: &Turn, number: usize) -> fmt::Result {
    let model_time = turn
        .model_duration_ms
        .map_or_else(|| "unavailable".to_owned(), duration);
    write!(
        html,
        r#"<div class="turn"><div class="turn-label"><span>Turn {number}</span><small>active {} · wall {} · {}</small></div><div class="track" role="img" aria-label="Turn {number}: wall {}; active {}; model {}; tools {}" title="Wall: {}; active: {}; model: {}; tools: {}">"#,
        duration(turn.duration_ms),
        duration(turn.wall_duration_ms),
        turn.status.label(),
        duration(turn.wall_duration_ms),
        duration(turn.duration_ms),
        model_time,
    ...
    for segment in &turn.tool_segments {
        let visible_offset = segment.offset_ms.min(turn.wall_duration_ms);
        let visible_duration = segment
            .duration_ms
            .min(turn.wall_duration_ms.saturating_sub(visible_offset));
        let left = percent(visible_offset, turn.wall_duration_ms);
        let width = percent(visible_duration, turn.wall_duration_ms)
            .max(1)
            .min(10_000_u64.saturating_sub(left));
        write!(
            html,
...
fn render_diagnostics(html: &mut String, session: &Session) -> fmt::Result {
    ...
    diagnostic(html, "Unestimated turns", diagnostics.unestimated_turns)?;
    write!(
        html,
    ...
    if !diagnostics.timing_errors.is_empty() {
        html.push_str("<h4>Timing inconsistencies</h4><ul class=parse-errors>");
        for error in &diagnostics.timing_errors {
            write!(html, "<li>{}</li>", escape(error))?;
        }
        html.push_str("</ul>");
    }
...
const fn anomaly_count(diagnostics: &Diagnostics) -> usize {
    diagnostics
        .parse_errors
        .len()
        .saturating_add(diagnostics.timing_errors.len())
...
mod tests {
    ...
    use super::{anomaly_count, duration, escape, render, render_turn};
    use crate::trace::{
    ...
    #[test]
    fn renders_tools_against_wall_without_inventing_model_time() -> Result<(), Box<dyn Error>> {
        let started_at = "2026-01-01T00:00:00Z".parse::<Timestamp>()?;
        let ended_at = "2026-01-01T00:00:05Z".parse::<Timestamp>()?;
        let turn = Turn {
            duration_ms: 1_000,
            ended_at,
            id: "inconsistent".to_owned(),
        ...
        render_turn(&mut html, &turn, 1)?;

        assert!(html.contains("model unavailable"));
        assert!(html.contains("left:20.00%;width:20.00%"));
        assert!(html.contains("active 1.0 s · wall 5.0 s"));
    ...
    fn renders_complete_dashboard_contract() -> Result<(), Box<dyn Error>> {
        ...
        let diagnostics = Diagnostics {
            ...
            timing_errors: vec!["turn <1>: model unavailable".to_owned()],
            trace_bytes: 42,
            unestimated_turns: 1,
            ..Diagnostics::default()
        };
        ...
        for expected in [
            ...
            "Turn containment timeline",
            "active 3.0 s · wall 5.0 s",
            r"<span>Model</span><strong>—</strong><small>unavailable · 1 turn</small>",
